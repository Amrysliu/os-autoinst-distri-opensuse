...
# Summary: testsuite python3-tools
# - verify the systems python3-tools and python3-pip
# - creating and installing source distribution package using pip
# - install available python3-tools via python3 installation.
# - verify the available python3-tools with creation of package locally.
...
sub run {
    ...
    assert_script_run('curl -L -s ' . data_url('python/python3-tools') . ' | cpio --make-directories --extract && cd data');
...
sub run_tests ($python3_spec_release) {
    return unless script_run("zypper search $python3_spec_release-tools") == 0;
    zypper_call("install $python3_spec_release $python3_spec_release-tools");
    record_info("pip3 version:", script_output("rpm -q $python3_spec_release-pip"));
    record_info("python3-tools:", script_output("rpm -q $python3_spec_release-tools"));
...
sub local_install_test ($version_number) {
    validate_script_output("pip$version_number install dist/user_package_tools-1.0.tar.gz", sub { m/Successfully installed/ });
    my @package_loc = split(/:/, script_output("pip$version_number show user_package_tools | grep Location"));
    validate_script_output("ls $package_loc[1]", sub { m/user_package_tools/ });
...
sub http_install_test ($version_number) {
    ...
    assert_script_run "mkdir -p repo_webroot/user-package-tools && cp dist/*.tar.gz repo_webroot/user-package-tools && pushd repo_webroot";
    ...
    assert_script_run "pip$version_number install --no-build-isolation -i http://localhost:8000 user_package_tools";
...
# Uninstall the installed user_package_tools and verify it.
sub uninstall_package ($version_number) {
    assert_script_run("pip$version_number uninstall user_package_tools -y");
...
sub cleanup {
    ...
    assert_script_run("rm -rf dist user_package_tools.egg-info repo_webroot");
